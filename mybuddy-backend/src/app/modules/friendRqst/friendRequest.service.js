...
export const getAllFriendRequestService = async (id) => {
  const friendRequests = await FriendRequest.find({ requestedBy: id })
...
export const getPendingFriendRequestService = async (id) => {
  const friendRequests = await FriendRequest.find({ requestedTo: id })
    .find({ status: "Pending" })
    .populate("requestedBy")
    .populate("requestedTo")
    .sort({ createdAt: -1 });
...
export const getAcceptedFriendRequestService = async (id) => {
  try {
    const friendRequests = await FriendRequest.find({
      status: "Accepted",
      $or: [{ requestedBy: id }, { requestedTo: id }],
    })
      .populate("requestedBy")
      .populate("requestedTo")
      .sort({ createdAt: -1 });
    ...
    throw new Error(
      "Error fetching accepted friend requests: " + error.message
    );
  }
};
