import {
  useDeletePayoneerFundRequestMutation,
  useGetAllPayoneerFundInfoQuery,
  useUpdatePayoneerFundStatusMutation,
} from "../../../features/payoneerfund/payoneerFundApi";
...
const PayoneerFund = () => {
  ...
  const payoneers = getAllPayoneerFundInfo?.data;
  const [updatePayoneerFundStatus] = useUpdatePayoneerFundStatusMutation();
  const [deletePayoneerFundRequest] = useDeletePayoneerFundRequestMutation();
  ...
  useEffect(() => {
    if (getAllPayoneerFundInfo?.data) {
      const initialStatuses = getAllPayoneerFundInfo?.data.reduce(
        (acc, fund) => {
  ...
  const handleStatusChange = async (fund, status) => {
    ...
    const selectedFund = payoneers?.find((perFund) => perFund?._id === fundId);
    ...
    if (result.isConfirmed) {
      try {
        ...
        await updatePayoneerFundStatus({
