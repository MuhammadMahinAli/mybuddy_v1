...
const PaypalFund = () => {
  ...
  const { data: getAllPaypalFundInfo, isLoading } = useGetAllPaypalFundInfoQuery();
  ...
  return (
    <div>
      {/* Header */}
      <div className="w-full py-4 bg-[#e9f2f9] flex my-5 items-center shadow rounded-xl">
        <div className="text-center w-4/12 md:w-3/12 font-semibold">Payment</div>
        <div className="text-center md:w-2/12 lg:w-3/12 hidden md:block font-semibold">Project</div>
        <div className="text-center md:w-2/12 lg:w-3/12 hidden lg:block font-semibold">Details</div>
        <div className="text-center md:w-2/12 lg:w-2/12 hidden md:block font-semibold">Amount</div>
        <div className="text-center w-4/12 md:w-2/12 font-semibold">Status</div>
        <div className="text-center w-4/12 md:w-2/12 font-semibold">Action</div>
      </div>

      {/* PayPal Funds */}
      {paypals?.map((p) => (
        <div key={p?._id} className="w-full py-4 bg-[#e9f2f9] flex my-5 items-center shadow rounded-xl">
          <div className="w-4/12 md:w-3/12 flex flex-col md:flex-row justify-center items-center pl-3 text-lg border-r">
            <img src="https://i.ibb.co.com/NYDfhjQ/paypal-1.png" className="w-8 h-8 rounded-lg" alt="Paypal" />
            <p className="capitalize">Paypal</p>
          </div>
          <div className="hidden md:block md:w-2/12 lg:w-3/12 text-center border-r">{p.projectName}</div>
          <div onClick={() => handleOpenDetails(p)} className="cursor-pointer hidden lg:block md:w-2/12 lg:w-3/12 text-center border-r">View Details</div>
          <div className="hidden md:block md:w-2/12 lg:w-2/12 text-center border-r">${p.amount}</div>
          <div className="w-4/12 md:w-2/12 text-center border-r">
          <span
            ...
            {selectedFundId === p?._id && p.status === "Pending" && (
              <div className="relative">
                <ul className="absolute right-2 bg-white border rounded-lg shadow-lg mt-2 z-10">
                  <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer" onClick={() => handleStatusChange(p, "Done")}>Done</li>
                  <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer" onClick={() => handleStatusChange(p, "Declined")}>Decline</li>
                </ul>
              </div>
            )}
          </div>
