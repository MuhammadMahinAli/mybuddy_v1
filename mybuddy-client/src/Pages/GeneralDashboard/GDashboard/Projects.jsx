...
const Projects = () => {
  ...
  <div className="flex justify-between items-center lg:hidden">
    {projects?.length === 1 ? (
      ...
      <ProjectCard p={projects[0]} i={projects[0]?._id} className="w-full" />
    ) : projects?.length === 2 ? (
      // When there are 2 projects, show with slider arrows
      <>
        <IoIosArrowBack className="text-3xl cursor-pointer" onClick={handlePrev} />
        <ProjectCard p={getAllProjectByUser?.data[currentIndex]} i={getAllProjectByUser?.data[currentIndex]?._id} className="w-[48%]" />
        <ProjectCard p={getAllProjectByUser?.data[currentIndex + 1]} i={getAllProjectByUser?.data[currentIndex + 1]?._id} className="w-[48%]" />
        <IoIosArrowForward className="text-3xl cursor-pointer" onClick={handleNext} />
      </>
    ) : (
  ...
  <div className="lg:flex justify-between items-center hidden">
    {getAllProjectByUser?.data?.slice(0, 3).map((project, index) => (
      <ProjectCard key={project._id} p={project} i={project._id} className="w-[32%]" />
    ))}
  </div>
</>
