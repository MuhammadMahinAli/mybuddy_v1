...
const UpdateTechnicalRecommendation = ({ handleACheckboxChange, formData }) => {
  const logos = [
    {
    ...
    <div className="w-full px-3">
      {logos.map((category) => (
        <div key={category.category} className="py-3 md:py-5 lg:py-7">
          <p className="py-4 text-lg text-start font-semibold capitalize">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3  gap-5 lg:gap-8 pt-3">
            {category.items.map((item) => (
              ...
              >
                <input
                  type="checkbox"
                  onChange={() => handleACheckboxChange(item.name)}
                  checked={formData?.technicalRecommendations.includes(
                    item.name
                  )}
                />
...
export default UpdateTechnicalRecommendation;
