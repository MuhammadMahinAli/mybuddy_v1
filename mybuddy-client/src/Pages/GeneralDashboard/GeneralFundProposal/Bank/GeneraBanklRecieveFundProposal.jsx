...
const GeneralBankRecieveFundProposal = () => {
  const { getBankFundByRequestedTo } = useContext(AuthContext);
  ...
  const allRecieveFundRequest = getBankFundByRequestedTo?.data;

  return (
    <div>
      {allRecieveFundRequest?.length === 0 ? (
        <p className="text-gray-600 text-[16px] lg:text-[24px] pb-5 font-medium text-center lg:text-start w-11/12 md:w-[600px] pt-7">{`You've not recieve any fund proposal yet.`}</p>
      ) : (
        <div className="w-[300px] xs:w-[330px] ssm:w-[370px] sm:w-[570px] md:w-[630px] lg:w-full overflow-x-auto xl:overflow-hidden">
          ...
          {allRecieveFundRequest?.map((fundRqst, index) => (
            ...
            >
              ...
              <div className="text-[16px] md:text-[18px] capitalize  border-r-2 border-white text-center w-2/12 lg:w-2/12">
...
export default GeneralBankRecieveFundProposal;

// const GeneraBanklRecieveFundProposal = () => {
//   return (
//     <div>
//       GeneraBanklRecieveFundProposal
//     </div>
//   );
// };

// export default GeneraBanklRecieveFundProposal;
