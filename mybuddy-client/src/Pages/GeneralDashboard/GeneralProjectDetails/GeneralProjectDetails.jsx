...
import {
  useGetAllAcceptedProjectTeamMemberQuery,
...
const GeneralProjectDetails = () => {
  ...
  const filteredMyself2 = currentTeamMember?.filter((myself) =>
    console.log(
      myself.requestedBy._id,
      userId,
      myself.projectId._id,
      ProjectInfo?._id
    )
  );

  console.log("userId", filteredMyself);
  ...
  const handleSubmit = async (e) => {
    ...
    const postData = {
      ...formData,
      commitBy: userId, // Assuming userId is available in your context or props
      project: ProjectInfo?._id, // Replace with the actual project ID
      status: "Pending",
    };
  
    try {
      await createCommit(postData); // Replace with your actual function for creating the commit
      Swal.fire({
        icon: "success",
 ...
 <td className="px-4 py-4 text-gray-800">
  ...
  <td className="px-4 py-4">
   <StatusDropdown commitId={commit?._id} getAllCommit={getAllCommit} />
   </td> 
 </tr>
  ))
 } 
