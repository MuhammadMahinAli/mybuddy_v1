...
import { useDeletePaypalLinkMutation, useUpdatePaypalLinkMutation } from "../../../features/paypal/paypalApi";
import { useDeletePayoneerLinkMutation, useUpdatePayoneerLinkMutation } from "../../../features/payoneer/payoneerApi";


const SettingForm = () => {
  const { userId, getUsersPaypalLink , getUsersPayoneerLink } = useContext(AuthContext);
  ...
  const userPaypalLink = 
  getUsersPaypalLink?.data?.paypalLink || "";

  console.log("pppp",getUsersPaypalLink);

  const userPayoneerLink =
    getUsersPayoneerLink?.data?.payoneerLink || "";

    const userPaypalId = 
    getUsersPaypalLink?.data?._id;

  const userPayoneerId =
    getUsersPayoneerLink?.data?._id;
    ...
    <div className="bg-white p-8 rounded-lg shadow-lg w-[600px]">
      ...
      {userPaypalLink ? (
        ...
        !isOpenPaypalEdit ? (
          <input
            type="text"
            value={updatedPaypalLink?.length !== 0 ? updatedPaypalLink : userPaypalLink}
            readOnly
...
<div className="bg-white p-8 rounded-lg shadow-lg w-[600px] ">
      ...
      {userPayoneerLink ? (
        !isOpenPayoneerEdit ? (
          <input
            type="text"
            value={updatedPayoneerLink?.length !== 0 ? updatedPayoneerLink :userPayoneerLink}
            readOnly
