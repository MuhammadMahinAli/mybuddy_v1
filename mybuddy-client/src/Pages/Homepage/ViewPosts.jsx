...
//         `https://test-two-22w0.onrender.com/api/v1/posts/getAll?page=${page}&limit=5&excludeIds=${fetchedPostIds.join(
//           ","
//         )}`
//       );
...
//           (post) => !fetchedPostIds.includes(post._id)
//         );
//         setPosts((prev) => {
//           const uniquePosts = [...prev];
//           newPosts.forEach((newPost) => {
//             if (!uniquePosts.some((post) => post._id === newPost._id)) {
//               uniquePosts.push(newPost);
//             }
//           });
//           return uniquePosts;
//         });
//         setFetchedPostIds((prev) => [...prev, ...newPosts.map((p) => p._id)]);
...
//   console.log(page, fetchedPostIds);
...
// //  if (!posts.length && !isLoading) return <div>No posts available</div>;
...
//   const [limit] = useState(5); // Number of posts to load per request
//   const [hasMore, setHasMore] = useState(true);
//   const [isLoading, setIsLoading] = useState(false);

//   const fetchPosts = async () => {
//     if (!hasMore || isLoading) return; // Avoid unnecessary calls

//     setIsLoading(true);
//     try {
//       const response = await fetch(
//         `https://test-two-22w0.onrender.com/api/v1/posts/getAll`
//       );
//       const data = await response.json();

//       if (data.success) {
//         setPosts((prevPosts) => [...prevPosts, ...data.data.posts]); // Append posts
//         setSkip(skip + data.data.posts.length); // Update skip
//         setHasMore(data.data.hasMore); // Check if there are more posts
//       }
//     } catch (error) {
