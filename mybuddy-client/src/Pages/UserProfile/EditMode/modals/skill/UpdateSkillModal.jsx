...
import {useUpdateSkillsMutation } from "../../../../../features/skill/skillApi.js";
import Swal from "sweetalert2";
import { AuthContext } from "../../../../../Context/UserContext.jsx";

const UpdateSkillModal = ({ isUpdateOpen, closeUpdateModal,allSkill }) => {
  const { singleUser,updateSkills } = useContext(AuthContext);
  const currentSkillArray = allSkill[0]?.skillArray;
  ...
  const [formData, setFormData] = useState({
    skillArray: [...currentSkillArray], // Initialize with current skills
  });

  const handleCheckboxChange = (skillName) => {
    setFormData((prevState) => {
      const skillArray = prevState.skillArray.includes(skillName)
        ? prevState.skillArray.filter((skill) => skill !== skillName) // Remove skill if it exists
        : [...prevState.skillArray, skillName]; // Add skill if it doesn't exist
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Good Job !",
        text: "You've Updated Your Skill !",
      });
      closeUpdateModal();
      setTimeout(() => {
  ...
  //if (initialError) return <div>Error loading skills</div>;
  const logos = [
    {
                ...
                <div className="px-[3px] rounded-m bg-white">
                  <div className="w-full py-5">
                    <div className="flex flex-col justify-center items-center">
                      ...
                      <span className="text-green-500">{userName}</span> update you skill from here !
                      </p>
                              ...
                              {category.items.map((item) => (
                                ...
                                >
                                  <input
                                    ...
                                    checked={formData.skillArray.includes(item.name)}
                                  />
