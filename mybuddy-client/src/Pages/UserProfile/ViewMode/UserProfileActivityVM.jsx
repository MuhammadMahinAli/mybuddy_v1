          ...
          <>
            ...
            {allPost?.length > 6 && (
              <div className="flex justify-center items-center py-5">
                {theme === "light" ? (
                  <div onClick={()=>setIsReadMoreOpen(!isReadMoreOpen)} className="w-44 flex items-center space-x-2 justify-center lg:text-sm xl:text-lg text-white font-semibold rounded-[10px] px-2 py-1 xl:px-4 xl:py-2 cursor-pointer bg-gradient-to-l from-[#2adba4] to-[#69f9cc]">
                    <p className="text-sm capitalize">   {isReadMoreOpen ? "View More" : "View Less"}</p>
                    <RightArrowIcon theme={theme} />
                  </div>
                ) : (
                  <div onClick={()=>setIsReadMoreOpen(!isReadMoreOpen)} className="seeAllPostBtn">
                    <p>
